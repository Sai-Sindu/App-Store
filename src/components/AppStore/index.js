...
class AppStore extends Component {
  ...
  getTabApps = () => {
    ...
    const filteredTabAppItems = appsList.filter(
      eachAppItem => eachAppItem.category === activeTab,
    )
    return filteredTabAppItems
  }

  render() {
    const {searchInput, activeTab} = this.state
    const filteredTabAppItems = this.getTabApps()

    const filteredAppItems = filteredTabAppItems.filter(eachAppItem =>
    ...
    return (
      <div className="app-store-container">
        ...
        <ul className="app-item-container">
          {filteredAppItems.map(eachAppItem => (
            <AppItem
              appsList={appsList}
              eachAppListDetails={eachAppItem}
              key={eachAppItem.appId}
            />
          ))}
        </ul>
      </div>
